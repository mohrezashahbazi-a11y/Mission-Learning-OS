// Mission Learning OS — Analytics Engine v1.1
...
(() => {
  ...
  function subjects(){const out={};history().forEach(x=>{const id=x.id||'unknown';if(!out[id])out[id]={id,sessions:0,minutes:0,plannedMinutes:0};out[id].sessions++;out[id].minutes+=Number(x.actualMinutes||0);out[id].plannedMinutes+=Number(x.plannedMinutes||0)});return Object.values(out)}
  function mastery(){return read(MASTERY)||{}}
  function snapshot(){return{version:'1.1',generatedAt:new Date().toISOString(),summary:summary(),last7Days:last7(),subjects:subjects(),mastery:mastery()}};
  window.missionOSAnalytics={version:'1.1',summary,last7Days:last7,subjects,mastery,snapshot};
})();
