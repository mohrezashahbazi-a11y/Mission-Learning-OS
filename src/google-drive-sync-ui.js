// Mission Learning OS — Cloud & Sync UI v1.2
(() => {
  function showCloud(){
  ...
  function render(){
    ...
    if(nav){
      ...
      b.onclick=e=>{e.preventDefault();e.stopPropagation();showCloud()};
      nav.addEventListener('click',e=>{const button=e.target.closest('button[data-page="cloud-sync"]');if(button){e.preventDefault();e.stopPropagation();showCloud()}},true);
    }
