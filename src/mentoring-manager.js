...
(() => {
  ...
  const load = () => {
    try {
      ...
      return Array.isArray(data) ? data.sort((a,b) => a.number - b.number) : INITIAL.slice();
    } catch { return INITIAL.slice(); }
  };
  const save = rows => localStorage.setItem(KEY, JSON.stringify(rows));
  const fmt = date => new Date(date + 'T00:00:00').toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' });
  const daysUntil = date => Math.max(0, Math.ceil((new Date(date + 'T23:59:59') - new Date()) / 86400000));
  ...
  function ensurePanel() {
    ...
    panel.id = 'mentoringManager';
    panel.style.marginBottom = '18px';
    timeline.insertBefore(panel, timeline.firstElementChild);
    return panel;
  }
  ...
  function render() {
    ...
    const rows = load();
    panel.innerHTML = `
      <div class="eyebrow">Mentoring</div>
      <h2 style="margin-top:5px">Mentoring Sessions</h2>
      <div class="subtitle" style="margin-bottom:14px">You control the dates. Add the next session whenever it is confirmed; the Director will use the nearest upcoming session as a strategic checkpoint.</div>
      <div id="mentorRows"></div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:14px">
        <input id="mentorDate" type="date" style="background:#0d1523;border:1px solid var(--line);border-radius:9px;padding:9px 10px">
        <button class="primary" id="addMentor">Add next session</button>
      </div>`;
    ...
    document.getElementById('addMentor').onclick = () => {
    ...
    panel.querySelectorAll('.mentorDelete').forEach(btn => btn.onclick = () => {
      const current = load();
