...
(() => {
  ...
  function ensureReviewPlan(id){
    ...
    state.reviews[id]={intervals:INTERVALS.map((interval,i)=>{const due=new Date(completed);due.setDate(due.getDate()+interval);return {reviewNumber:i+1,intervalDays:interval,dueDate:key(due),status:'pending'};})};
  }
  function dueReviews(){
    const debug=new URLSearchParams(location.search).get('reviewDebug')==='1';
    const out=[];
    for(const m of MISSIONS){
      ensureReviewPlan(m.id);const plan=state.reviews[m.id];if(!plan)continue;
      plan.intervals.forEach(r=>{if(r.status==='pending'&&(debug||r.dueDate<=key(now())))out.push({id:`REVIEW-${m.id}-${r.reviewNumber}`,isReview:true,sourceId:m.id,reviewNumber:r.reviewNumber,order:m.order,type:'review',title:`Review ${r.reviewNumber}: ${m.title}`,subject:m.subject,mins:Math.min(20,Math.max(10,Math.round(m.mins*.4))),energy:Math.min(4,m.energy||2),weight:10,priorityBoost:r.dueDate<key(now())?20:12,dueDate:r.dueDate,goal:`Retrieve the key ideas from ${m.title} without rereading first.`,why:`Scheduled spaced review (${r.intervalDays}-day interval).`,pre:'The source Mission is completed.',next:r.reviewNumber<5?`Next review in ${INTERVALS[r.reviewNumber]} days.`:'Continue retrieval only when the Director schedules it.',deliver:'Brief recall: explain the key ideas and answer the original completion check.',check:'You can explain the key ideas without looking at the source.'});});
    }
    return out;
  }
  function buildQueue(){
  ...
  function completeMission(m){
    if(m.isReview){const plan=state.reviews[m.sourceId];const r=plan?.intervals.find(x=>x.reviewNumber===m.reviewNumber);if(r)r.status='completed';save();render();toast('Review completed. Queue recalculated.');return;}
  ...
  function render(){
    const {q,used,p}=buildQueue();const set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v;};
    set('coreCount',q.length);set('queueCount',`${q.length} Core Missions · ${p.name}`);set('remaining',`${used}m`);set('done',state.done.length);set('pct',`${Math.round(state.done.length/MISSIONS.length*100)}%`);const bar=document.getElementById('bar');if(bar)bar.style.width=`${Math.round(state.done.length/MISSIONS.length*100)}%`;
